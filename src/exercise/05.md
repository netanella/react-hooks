...
The jsx is React.createElement under the hood so we have no access to DOM nodes
in our function components. The nodes are only created until ReactDOM.render is
called.

To get access, we use the ref prop.

The DOM interaction will be in a useEffect hook - because it is called when
after a component has been rendered.

we'll need to clean up if this component is unmounted to avoid event 
handlers on DOM nodes that are no longer in the document. This is wasting 
the users memory and can cause a memory leak and sub-optimal performance.

If we want a state update to cause a rerender - use useState. If we want to 
update without a rerender - use useRef.
