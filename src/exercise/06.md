...
We can't make a useEffect function async because it returns a promise. The
useEffect can only return the cleanup function. Instead, we can declare the
async function in the useEffect body and envoke it. Another option is to use
`.then()` method.

there are two ways to handle api errors: `.then(d => {...}).catch(e => {...})` /
`.then(d => {...}, e => {...},`. catch is generally safer because it catches any
error in `d => {...}`, not only fetch errors.

By using the "status" we avoid showing the error message / pokemon data when
they were not cleared.

Normally, React batches state setters together and rerender only once. Before
React 18, the batching wasn't supported in async functions.

Now this problem doesn't exist anymore but it is still a good practice to 
maintain closely related states as an object.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=netanella1%40gmail.co).
